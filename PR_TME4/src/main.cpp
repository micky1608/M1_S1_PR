...
//	}
	...
	micky::Pool pool((size_t)(scene.getHeight()*scene.getWidth()));
	micky::Barrier barrier(scene.getWidth()*scene.getHeight());

	for (int x =0 ; x < scene.getWidth() ; x++) {
		for (int y = 0; y < scene.getHeight(); y++) {
			auto *pixelJob = new micky::PixelJob(barrier, x, y, screen, scene, lights, pixels);
			pool.submit(pixelJob);
		}
	}
